...
const ExcelViewer: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event: ProgressEvent<FileReader>) => {
      ...
      if (arrayBuffer) {
        ...
        // Fill empty values with zero for display
  ...
  const countErrors = (data: any[][]) => {
    ...
    setErrorCount(count);
    if (count > 0) {
      alert(`Please make sure data is in correct format. Errors: ${count}. Remove unncessary spaces and special characters.`);
    }
  };

  const sendDataToBackend = async () => {
    try {
      const renderURL = `https://azure-embee-server.onrender.com/excel-data-ea`;
      // const renderURL=`http://4.240.47.21:3000/excel-data-ea`
      const body = {
  ...
  const formatNumber = (value: any, columnIndex: number) => {
    // Skip formatting for enrollment number (assumed column index 0)
    if (columnIndex === 0 || columnIndex==3) return value;
  ...
  return (
    <div className="p-4">
      <h1 className="text-3xl">Update EA Data</h1>
    ...
    {row.map((cell, cellIndex) => (
      <td
        ...
        className={`border border-gray-300 px-4 py-2 ${
          ...
          cellIndex === 3 && containsSpecialChars(cell) ? 'bg-red-500 text-white' : ''
        } ${
