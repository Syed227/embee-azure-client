...
const CompanyProfileEA: React.FC = () => {
  ...
  type MonthAbbreviation = 'jan' | 'feb' | 'march' | 'april' | 'may' | 'june' | 'july' | 'aug' | 'sep' | 'oct' | 'nov' | 'dec';
  async function initialiseDB() {
    var logicAppUrl = 'https://prod-10.centralindia.logic.azure.com:443/workflows/2bb26d06efee4ba1beec91b554ec5e86/triggers/manual/paths/invoke?api-version=2016-10-01&sp=%2Ftriggers%2Fmanual%2Frun&sv=1.0&sig=A4DgyAp3j-xtBhyQ6MGqyl0H3t_QXf5Y0WrcFUjzk3E';
    ...
    const fetchCompanyData = async () => {
      try {
        ...
        const [data, rankData, avgData] = await Promise.all([
          fetchData(`http://4.240.47.21:3000/getCompanyDataEA/${name}`),
          fetchData(`http://4.240.47.21:3000/getRankEA/${name}`),
          fetchData(`http://4.240.47.21:3000/getAverageEA`),
        ]);
...
// Example usage of insights
  ...
  const getRankChangeMessage = () => {
    ...
    if (currentRank.position > previousRank.position) {
      return `Previously, ${name} ranked at number ${previousRank.position} in all companies for their EA consumption in the month of ${monthBeforePrevious}, but has now fallen to rank ${currentRank.position} in the month of ${previousMonth}. Its consumption has decreased by ₹${formatNumber(Math.abs(consumptionChange))}.`;
    } else if (currentRank.position < previousRank.position) {
      return `Previously, ${name} ranked at number ${previousRank.position} in all companies for their EA consumption in the month of ${monthBeforePrevious}, but has now improved to rank ${currentRank.position} in the month of ${previousMonth}. Its consumption has increased by ₹${formatNumber(Math.abs(consumptionChange))}.`;
    } else {
...
export default CompanyProfileEA;
